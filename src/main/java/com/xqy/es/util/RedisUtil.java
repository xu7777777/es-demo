...
public class RedisUtil {
    ...
    public String get(String key) {
        return key == null ? null : String.valueOf(stringRedisTemplate.opsForValue().get(key));
    }
    ...
    public String hget(String key, String item) {
        return String.valueOf(stringRedisTemplate.opsForHash().get(key, item));
    }
     ...
     */
    public Long addGeoPoint(String k, Point point, String m) {
        Long addedNum = stringRedisTemplate.opsForGeo().add(k, point, m);
        return addedNum;
    }

    /**
     * 查找指定key的经纬度信息，可以指定多个，批量返回
     * redis命令：geopos cityGeo 北京
     */
    public List<Point> geoGet(String k, String... m) {
        List<Point> points = stringRedisTemplate.opsForGeo().position(k, m);
        return points;
    }
    ...
    public Distance geoDist(String k, String mk1, String mk2, Metric metric) {
        Distance distance = stringRedisTemplate.opsForGeo().distance(k, mk1, mk2, metric);
        return distance;
    }
    ...
    public void nearByXY(String k, Circle circle, RedisGeoCommands.GeoRadiusCommandArgs args) {
        ...
        GeoResults<RedisGeoCommands.GeoLocation<String>> results = stringRedisTemplate.opsForGeo().radius(k, circle, args);
...
//        RedisGeoCommands.GeoRadiusCommandArgs args = RedisGeoCommands.GeoRadiusCommandArgs.newGeoRadiusArgs().includeDistance().includeCoordinates().sortAscending().limit(5);
        GeoResults<RedisGeoCommands.GeoLocation<String>> results = stringRedisTemplate.opsForGeo().radius(k, mk, distance, args);
        return results;
    }
    ...
    public List geoHash(String k, String... mks) {
        List<String> results = stringRedisTemplate.opsForGeo().hash(k, mks);
        return results;
    }
     ...
     * 获取redis锁
     */
    public Boolean lock (String lockName, String lockId, Long expire) {
        Boolean lockRes = stringRedisTemplate.opsForValue().setIfAbsent(lockName, lockId, expire, TimeUnit.SECONDS);
        return lockRes;
    }
    ...
    public Boolean unlock(String lockName, String lockId) {
        String val = this.get(lockName).toString();
        if (val.equals(lockId)) {
            Boolean deleteRes = stringRedisTemplate.delete(lockName);
            return deleteRes;
        }
        return true;
    }
